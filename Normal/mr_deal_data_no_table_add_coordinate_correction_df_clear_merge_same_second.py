...
class Common:
    ...
    def init_ans(ans):
        ...
        if 'u_sinr' in ans:
            ans['u_sinr'] = np.nan
        if 'u_neighbor_cell_number' in ans:
            ans['u_neighbor_cell_number'] = 0
        # 处理邻区初始化
        for i in range(1, 9):
            nfreq_name = 'u_freq_n%d' % (i)
            npci_name = 'u_pci_n%d' % (i)
            nrsrp_name = 'u_rsrp_n%d' % (i)
            nrsrq_name = 'u_rsrq_n%d' % (i)
            nsinr_name = 'u_sinr_n%d' % (i)
            if nfreq_name in ans:
                ans[nfreq_name] = np.nan
            if npci_name in ans:
                ans[npci_name] = np.nan
            if nrsrp_name in ans:
                ans[nrsrp_name] = np.nan
            if nrsrq_name in ans:
                ans[nrsrq_name] = np.nan
            if nsinr_name in ans:
                ans[nsinr_name] = np.nan
        # 处理mr4g邻区初始化
        for i in range(1, 9):
            nfreq_name = 'u_freq_4g_n%d' % (i)
            npci_name = 'u_pci_4g_n%d' % (i)
            nrsrp_name = 'u_rsrp_4g_n%d' % (i)
            nrsrq_name = 'u_rsrq_4g_n%d' % (i)
        ...
        # 处理mr5g邻区初始化
        for i in range(1, 9):
            nfreq_name = 'u_freq_5g_n%d' % (i)
            npci_name = 'u_pci_5g_n%d' % (i)
            nrsrp_name = 'u_rsrp_5g_n%d' % (i)
            nrsrq_name = 'u_rsrq_5g_n%d' % (i)
    ...
    @staticmethod
    def deal_local(ans, _g):
        if "u_longitude" in _g:
            ans["u_longitude"] = _g["u_longitude"].median()
        if "u_latitude" in _g:
            ans["u_latitude"] = _g["u_latitude"].median()
        if "u_altitude" in _g:
            ans["u_altitude"] = _g["u_altitude"].median()
        return ans

    @staticmethod
    def deal_main(ans, _g):
        # 处理主小区
        def deal_main_in(_g):
            main_dict = {}
            ...
            def main_deal(_r):
                ...
                if 'u_pci' in _r:
                    ...
                    if 'u_freq' in _r:
                        freq = int(_r['u_freq'])
                        if 'u_sinr' in _r:
                            val = [_r['u_rsrp'], _r['u_rsrq'], _r['u_sinr']]
                        else:
                            val = [_r['u_rsrp'], _r['u_rsrq']]
                        key = '%d_and_%d' % (pci, freq)
                        if not key in main_dict:
                            main_dict[key] = []
                        main_dict[key].append(val)

            _g.apply(main_deal, axis=1)
            max = 0
            max_key = ''
            for k, v in main_dict.items():
                ...
                if len_of_val > max:
                    ...
                    max_key = k
            if max > 0:
                lst = max_key.split("_and_")
                return_pci = int(lst[0])
                return_freq = int(lst[1])
                data = main_dict[max_key]
                if len(data[0]) == 3:
                    new_v = pd.DataFrame(data=data, columns=['rsrp', 'rsrq', 'sinr']).median().round(2).to_list()
                else:
                    new_v = pd.DataFrame(data=data, columns=['rsrp', 'rsrq']).median().round(2).to_list()
                ans = [return_pci, return_freq]
                for v in new_v:
                    ans.append(v)
                if len(ans) == 4:
                    ans.append(None)
        ...
        tmp = deal_main_in(_g)
        ans[['u_pci', 'u_freq', 'u_rsrp', 'u_rsrq', 'u_sinr']] = tmp
        return ans

    @staticmethod
    def deal_near(ans, _g):
        near = {}
                ...
                if npci_name in _r and not pd.isnull(_r[npci_name]):
                    ...
                    if nfreq_name in _r and not pd.isnull(_r[nfreq_name]):
                        ...
                        if nsinr_name in _r:
                            ...
                            val = [_r[nrsrp_name], _r[nrsrq_name]]
                        key = '%d_and_%d' % (pci, freq)
                        if not key in near:
                            near[key] = []
                        near[key].append(val)

        _g.apply(near_deal, axis=1)
        dealed_near = {}
        for k, v in near.items():
            if len(v[0]) == 3:
                new_v = pd.DataFrame(data=v, columns=['rsrp', 'rsrq', 'sinr']).median().round(2).to_list()
            else:
                new_v = pd.DataFrame(data=v, columns=['rsrp', 'rsrq']).median().round(2).to_list()
            lst = k.split("_and_")
            return_pci = int(lst[0])
            return_freq = int(lst[1])
            val = [return_pci, return_freq]
            for value in new_v:
                val.append(value)
            dealed_near[new_v[0]] = val

        sort_data = sorted(dealed_near.items(), key=lambda d: d[0], reverse=True)
        near_number = len(sort_data)
        for i in range(near_number):
            point = i + 1
            nfreq_name = 'u_freq_n%d' % (point)
            npci_name = 'u_pci_n%d' % (point)
            nrsrp_name = 'u_rsrp_n%d' % (point)
            nrsrq_name = 'u_rsrq_n%d' % (point)
            nsinr_name = 'u_sinr_n%d' % (point)
            key, val = sort_data[i]
            if len(val) == 4:
                ans[[npci_name, nfreq_name, nrsrp_name, nrsrq_name]] = val
            else:
                ans[[npci_name, nfreq_name, nrsrp_name, nrsrq_name, nsinr_name]] = val
        ans['u_neighbor_cell_number'] = near_number
        return ans

    @staticmethod
    def deal_mr_4g(ans, _g):
        near = {}
        ...
        for i in range(near_number):
            point = i + 1
            nfreq_name = 'u_freq_4g_n%d' % (point)
            npci_name = 'u_pci_4g_n%d' % (point)
            nrsrp_name = 'u_rsrp_4g_n%d' % (point)
            nrsrq_name = 'u_rsrq_4g_n%d' % (point)
            nsinr_name = 'u_sinr_4g_n%d' % (point)
            key, val = sort_data[i]
            if len(val) == 4:
                ans[[npci_name, nfreq_name, nrsrp_name, nrsrq_name]] = val
            else:
    ...
    def deal_mr_5g(ans, _g):
        ...
        for i in range(near_number):
            point = i + 1
            nfreq_name = 'u_freq_5g_n%d' % (point)
            npci_name = 'u_pci_5g_n%d' % (point)
            nrsrp_name = 'u_rsrp_5g_n%d' % (point)
            nrsrq_name = 'u_rsrq_5g_n%d' % (point)
            nsinr_name = 'u_sinr_5g_n%d' % (point)
            key, val = sort_data[i]
            if len(val) == 4:
                ans[[npci_name, nfreq_name, nrsrp_name, nrsrq_name]] = val
            else:
    ...
    def group_fun(_g):
        ans = _g.head(1)
        ans = Common.init_ans(ans)
        ans = Common.deal_local(ans, _g)
        ans = Common.deal_main(ans, _g)
        ans = Common.deal_near(ans, _g)
        ans = Common.deal_mr_4g(ans, _g)
        ans = Common.deal_mr_5g(ans, _g)
        return ans

    @staticmethod
    def do_one_second(df):

        ans = df.groupby('u_time').apply(Common.group_fun).reset_index(drop=True)
        ans = ans.sort_values(by='u_time').reset_index(drop=True)
        return ans
    ...
    @staticmethod
    def df_clear(in_df):
        print_with_line_number(f'开始数据清理')
        if 'f_sinr' in in_df.columns:
            print_with_line_number(f'清理 5G finger 数据')
            in_df = in_df.dropna(subset=['f_longitude', 'f_latitude', 'f_pci', 'f_freq', 'f_rsrp', 'f_rsrq', 'f_sinr'],
                           how='any')
        elif 'u_sinr' in in_df.columns:
            print_with_line_number(f'清理 5G UEMR 数据')
            ...
            in_df = in_df.dropna(subset=['f_longitude', 'f_latitude', 'f_pci', 'f_freq', 'f_rsrp', 'f_rsrq'], how='any')
        elif 'u_longitude' in in_df.columns and 'u_sinr' not in in_df.columns:
...
class DealData:
    ...
    def deal_walk_tour(self):
        ...
        if 'uemr'.lower() == self.config.get_data_type().lower():
            ...
            # 同秒合并
            # zcy_ue_merge_df = Common.do_one_second(zcy_ue_merge_df)
    ...
    def deal_wetest(self):
        ...
        if 'uemr'.lower() == self.config.get_data_type().lower():
            ...
            # res_ue_df = Common.do_one_second(res_ue_df)
